...
Fusepm.prototype.verify_module_unoproj = function verify_module_unoproj (fpath) {
	...
	var files = this.find_unoproj(fpath, 1, new RegExp(/\.unoproj$/), false);
	if (files.length == 1) {
		var unoproj = path.normalize(path.join(process.cwd(), fpath, files[0]));
		return unoproj;
	}
	files = this.find_unoproj(fpath, 1, new RegExp(/include\.unoproj$/), false);
	if (files.length < 1) {
		return this.find_package(fpath);
...
Fusepm.prototype.find_package = function find_package (fpath) {
	var files = fs.readdirSync(path.join(fpath , 'src'));
	var projects = [];
	for (var i = 0; i < files.length; i++) {
		var dirname = path.join(fpath, 'src', files[i]);
		var stats = fs.statSync(dirname);
		if (stats.isDirectory()) {
			var proj = this.find_unoproj(dirname, 1, new RegExp(/\.unoproj$/), false);
			if (proj.length === 1) {
				var unoproj = path.normalize(path.join(process.cwd(), dirname, proj[0]));
				projects.push(unoproj);
			}
		}
	}
	...
	if (projects.length > 0) {
		throw new Error('Found several projects. This is unsupported at this time');
		return projects;
	}
	throw new Error('Unable to find packages, is this a Fuse module?');
}

Fusepm.prototype.find_unoproj = function find_unoproj (fpath, count, re, fatal) {
...
Fusepm.prototype.local_unoproj = function local_unoproj(lpath) {
	...
	var re;
	if (this.commander && this.commander.unoproj) {
...
Fusepm.prototype.addto_unoproj = function addto_unoproj (fn, addobj) {
	...
	function ensure (obj, key, add) {
		if (!obj[key]) {
			obj[key] = [add];
		} else if (obj[key].indexOf(add) < 0)
			obj[key].push(add);
	}

	debug("addto_unoproj " + fn);
	debug(addobj);
	var self = this;
	return self.read_unoproj(fn).then(function (obj) {
		if (addobj.Projects) {
			ensure(obj, "Projects", addobj.Projects);
		}
		if (addobj.Excludes) {
			ensure(obj, "Excludes", addobj.Excludes);
		}
		if (addobj.Includes) {
			ensure(obj, "Includes", addobj.Includes);
		}
		if (addobj.FusePM && addobj.FusePM.Dependencies) {
			if (!obj.FusePM) {
				obj.FusePM = {};
			}
			ensure(obj.FusePM, "Dependencies", addobj.FusePM.Dependencies);
		}
		if (addobj.Version) {
...
Fusepm.prototype.save_to_unoproj = function save_to_unoproj (fn, addobj) {
	...
	debug(addobj);
	var self = this;
	return self.addto_unoproj(fn, addobj)
		.then(function (obj) {

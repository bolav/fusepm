...
Fusepm.prototype.addto_unoproj = function addto_unoproj (addobj) {
	debug("addto_unoproj");
	var self = this;
	return self.read_unoproj(local_name).then(function (obj) {
		if (addobj.Projects) {
			if (!obj.Projects) {
		...
		if (addobj.Excludes) {
			...
			} else if (obj.Projects.indexOf(addobj.Excludes) < 0)

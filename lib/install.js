...
function install_to_file(fn, package) {
	console.log("Installing dependency " + package);
	debug("install_to_file: " + fn + ", " + package);
	...
	if (parsed) {
		npm.load(conf, function (er) {
			...
			cloneRemote(parsed.path(), parsed.toString(), install_path, function (error) { if (error) throw error; postinstall(package, fn, install_path) });
			return;
		});
		return;
	}

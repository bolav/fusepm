	...
	fusepm.verify_local_unoproj(".").then(function (fn) {
		...
		if (parsed) {
			npm.load(conf, function (er) {
				...
				cloneRemote(parsed.path(), parsed.toString(), install_path, function (error) { if (error) throw error; postinstall(install_path) });
				return;
			});
			return;
		}
...
function postinstall (install_path) {
	...
	fusepm.verify_local_unoproj(".").then(function (local_unoproj) {
		fusepm.save_to_unoproj(local_unoproj, {Projects: unoproj, Excludes: 'fuse_modules/' });
	});
}
